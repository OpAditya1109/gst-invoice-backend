 ...
 */
export async function extractPages(file, pageOptions) {
    ...
    if (pageOptions.pageIndexes.length > pageCount) {
        errorHandler.throw(new MindeeError("The total indexes of pages to cut is superior to the total page count of the file (" +
            pageCount +
            ")."));
    }
    const pageIndexes = [];
    ...
    if (!pageIndexes.every((v) => currentPdf.getPageIndices().includes(v))) {
        errorHandler.throw(new MindeeError(`Some indexes pages
        (${pageIndexes.join(",")})
