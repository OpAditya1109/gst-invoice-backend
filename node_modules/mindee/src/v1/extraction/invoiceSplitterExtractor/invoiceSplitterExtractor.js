...
async function splitPdf(pdfDoc, invoicePageGroups) {
    ...
    for (let i = 0; i < invoicePageGroups.length; i++) {
        ...
        generatedPdfs.push(new ExtractedInvoiceSplitterImage(subdocumentBytes, [invoicePageGroups[i][0], invoicePageGroups[i][invoicePageGroups[i].length - 1]]));
    }
 ...
 */
export async function extractInvoices(inputFile, indexes, strict = false) {
    if (!indexes) {
